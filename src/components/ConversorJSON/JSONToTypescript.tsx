...
import {
	TypescriptInterfaceOrType,
	TypescriptParsingOptions,
} from "../../services/typescript-json-parser";
	...
	return (
		<>
			...
			<TextInput
				...
				onChange={({ target: { value: rootObjectName } }) =>
					setFormValues({ ...formValues, rootObjectName })
				}
			/>
			<Space h="md" />
			...
			<Switch
				...
				onChange={({ target: { checked: createObjectsAsTypes } }) =>
					setFormValues({ ...formValues, createObjectsAsTypes })
				}
			/>
			<Space h="md" />
			...
			<Switch
				...
				onChange={({ target: { checked: considerNullValuesAsOptional } }) =>
					setFormValues({ ...formValues, considerNullValuesAsOptional })
				}
			/>
			<Space h="md" />
			...
			<Switch
				...
				onChange={({
					target: { checked: createEachObjectAsASeparateInstance },
				}) =>
					setFormValues({
						...formValues,
						parseAsSingleInterface: createEachObjectAsASeparateInstance,
					})
				}
			/>
			...
			<Space h="md" />
			<Group>
				<Button disabled={!formValues.json} onClick={handleOpenResult}>
					Convert
				</Button>
				<Button
					variant="subtle"
...
const parseJson = (json: string, options: TForm) => {
	try {
		...
		return options.parseAsSingleInterface
			? TypescriptInterfaceOrType.fromJsonToSingleInstance(
					result,
					options?.rootObjectName ?? "IMain",
					options
			  )
			: TypescriptInterfaceOrType.fromJsonToMultipleInstances(
					result,
					options.rootObjectName ?? "IMain",
					options
			  );
