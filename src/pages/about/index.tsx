			...
			>
				...
				{isSuccess && (
					<>
						...
						{!!profile?.repos_url && (
							<Info
								label="My Github Repositores"
								value={<LinkDrawerRepositories {...profile} />}
							/>
						)}
						...
						<InfoGroup>
							...
							{!!profile?.following && (
								<Info label="Following (Github)" value={profile.following} />
							)}
							{!!profile?.followers && (
								<Info label="Followers (Github)" value={profile.followers} />
							)}
						</InfoGroup>
...
const LinkDrawerRepositories = ({ repos_url }: IGithubProfile) => {
	...
	return (
		<>
			<Anchor onClick={() => toggle(true)}>{repos?.length} public</Anchor>
			<Drawer
				padding="xl"
				position="right"
				size="xl"
...
const InfoGroup = ({ children }: { children: ReactNode }) => {
	return (
		<Flex gap={36} align="center" justify="flex-start" w="100%">
			{children}
		</Flex>
	);
};
...
const Info = ({ label, value }: { label: ReactNode; value: ReactNode }) => {
	return (
		<Flex direction="column" gap={2} align="flex-start" mb={8}>
